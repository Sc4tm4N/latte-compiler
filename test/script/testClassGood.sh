...
CTR=0
FLD=0
for filename in test/good/automatic/extension/class/without_input/ret_zero/*.lat; do
...
echo "GOOD - automatic - class - without input - ret zero -> Failed: $FLD/$CTR"


CTR=0
FLD=0
for filename in test/good/automatic/extension/class/with_input/ret_zero/*.lat; do
  ./latc $filename > /dev/null 2> /dev/null
  compile_result=$?
  executable=$(echo $filename | rev | cut -c5- | rev)
  your_output="$executable.gen_out"
...
echo "GOOD - automatic - mixed - with input - ret zero -> Failed: $FLD/$CTR"